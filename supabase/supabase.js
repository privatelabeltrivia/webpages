...
export async function getTeamIdByEmail(eventId, email) {
...
export async function getTeamNameByEmail(eventId, email) {
  const { data, error } = await supabase
    .from("Teams") // Replace with your actual table name
    .select("TeamName")
    .eq("Email", email)
...
export async function insertScoringRoundSubmission(payload) {
  const venueId = await getVenueIdByVenueTag(payload.venueid);
  const eventId = await getEventIdByVenueId(venueId);
  const teamId = await getTeamIdByEmail(eventId, payload.formdata[0]);

  for (let i = 1; i <= payload.formdata.length; i++) {
    const cleanPayload = {
      EventId: eventId,
      TeamId: teamId,
      RoundNum: payload.roundid,
      QuestionNum: i,
      SubmittedAnswer: payload.formdata[i]
    };

    console.log(
      "Inserting scoring round submission with payload:",
      cleanPayload,
    );
    const { data, error } = await supabase
