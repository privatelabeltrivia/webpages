...
const STORAGE_KEYS = {
    ...
    getVenueKeys: (venueId) => ({
        ...
        LOGOSRC: `plt_venue_${venueId}_header_src`,
        PRIMARY_COLOR: `plt_venue_${venueId}_primary_color`,
    }),
    getRoundPrefix: (venueId, roundNum) =>
        `plt_round_${venueId}_${roundNum}_`,
};
...
const KEY_REGISTRATION_TIME = "registration_time";
const KEY_TEAM_NAME = "team_name";
const KEY_DOUBLE_ROUND = "bonus_round";
const KEY_COMPLETED_ROUNDS = "completed_rounds";
const KEY_LOGOSRC = "logo_src";
...
function renderPage(data) {
    ...
    const roundTitleContainer = document.getElementById(
...
function renderNavPills(data) {
    ...
    const completedRounds = CacheManager.get(
        venueKeys.COMPLETED_ROUNDS,
        [],
    );
...
function restoreFormFromCache() {
    ...
    form.querySelectorAll(".q-block").forEach((block) => {
        ...
        console.log(
            `Restoring ${field.name} from cache with key ${key}:`,
            cached,
        );
...
async function getFormData() {
    try {
        ...
        if (!venueId || round === null) {
            showError("Missing venue or round information in URL");
            return;
        }
...
function toggleLoading(show, duration = 60000, callback = null) {
    ...
    if (!loadingLogo.src) loadingLogo.src = logoSrc;
...
function updateDuplicateStates() {
    ...
    if (selects.length === 0) {
        console.warn(
            "updateDuplicateStates: No <select> elements found in form.",
        );
        return;
    }
    if (!selects[0].options) {
        console.warn(
            "updateDuplicateStates: First select element has no options.",
        );
        return;
    }
    ...
    selects.forEach((select) => {
        const isDuplicated =
            select.value !== "" && valueCounts[select.value] > 1;
        ...
        select.style.backgroundColor = isDuplicated
            ? "#fffdf0"
            : "transparent";
...
(function () {
    ...
    window.addEventListener("load", () => {
        const regTime = parseInt(
            CacheManager.get(venueKeys.REGISTRATION_TIME),
            10,
        );
    ...
    form.addEventListener("submit", (e) => {
        ...
        const emptyFields = elementsToCheck.filter((el) => {
            ...
            const isContinuousClue =
                label &&
                label.textContent.toLowerCase().includes("continuous clue");
    ...
    function proceedWithSubmission() {
        ...
        try {
            ...
            console.error(
                "FormData iteration failed, using manual field mapping.",
            );
    ...
    function handleRedirect() {
        ...
        if (
            currentRound === 0 &&
            (nextRound === undefined || nextRound === null)
        ) {
